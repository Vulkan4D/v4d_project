...
private:
	...
	static const std::map<std::string, Element*> elementsBySymbol;
public:
	static const Element* Get(std::string symbol) {
		if (std::find(elementsBySymbol.begin(), elementsBySymbol.end(), symbol) == elementsBySymbol.end())
			return nullptr;
		return elementsBySymbol.at(symbol);
	}
	static const Element* Get(int8_t atomicNumber) {
		if (atomicNumber > elements.size() - 1)
			return nullptr;
...
struct Molecule {
	...
	std::vector<std::tuple<int8_t, int8_t>> elements;
	
	Molecule(std::vector<std::tuple<int8_t, int8_t>> elems) : elements(elems) {}
	
	Molecule(std::string formula) : elements({}) {
		// Caching for faster decoding
		if (std::find(molecules.begin(), molecules.end(), formula) != molecules.end()) {
			elements = molecules[formula].elements;
			return;
		}
		if (formula.length() < 3) {
			auto* singleElement = Element::Get(formula);
			if (singleElement) {
		...
		do {
			...
			tmp = m[3].str();
			try {
				elements.emplace_back(element->atomicNumber, (int8_t)atoi(n.c_str()));
			} catch (...) {
				elements.clear();
				return;
			}
...
const auto Element::elementsBySymbol {
...
std::map<std::string, Molecule> Molecule::molecules {
	_DEFINE_MOLECULE(H2)
	_DEFINE_MOLECULE(N)
	_DEFINE_MOLECULE(O2)
