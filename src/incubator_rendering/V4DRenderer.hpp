...
private: // Graphics configuration
	...
	void CreatePipelines() override {
		...
		{// Main Render pass
			...
			renderPass.CreateFrameBuffers(renderingDevice, *mainCamera.GetTmpImage());
			
			// Shaders
			for (auto* shaderPipeline : {&galaxyBoxShader}) {
				shaderPipeline->SetRenderPass(mainCamera.GetTmpImage(), renderPass.handle, 0);
...
private: // Commands
	...
	void RecordGraphicsCommandBuffer(VkCommandBuffer commandBuffer, int imageIndex) override {
		...
		// Main render pass
		renderPass.Begin(renderingDevice, commandBuffer, *mainCamera.GetTmpImage(), {{.0,.0,.0,.0}, {1.0,.0}});
...
public: // Update
	...
	void LowPriorityFrameUpdate() override {
		...
		// Update UBO
		GalaxyUBO ubo {
			glm::dvec4(mainCamera.GetWorldPosition(), 1),
			galaxyFrameIndex
		};
