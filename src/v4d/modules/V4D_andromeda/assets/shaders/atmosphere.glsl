...
layout(buffer_reference, std430, buffer_reference_align = 4) buffer AtmosphereData {
	float planetSolidRadius;
...
void main() {
	AtmosphereData atmosphereData = AtmosphereData(GetCustomData());
	...
	if (rayDepth < atmosphereData.minStepSize) return;
	...
	if (stepSize < atmosphereData.minStepSize) {
		rayMarchingSteps = int(ceil(rayDepth / atmosphereData.minStepSize));
		...
		if (light.radius > 10000000.0) { // 10'000 km
			...
			if (luminosity > sunLuminosityThreshold) {
				...
				for (float dist = 0; dist < rayDepth*0.999; dist += stepSize) {
					...
					float rayAltitudeAboveInnerRadius = rayAltitude - atmosphereData.innerRadius;
					...
					vec2 density = exp(-max(0.001, rayAltitudeAboveInnerRadius) / vec2(atmosphereData.rayleighHeight, atmosphereData.mieHeight)) * stepSize;
					opticalDepth += density;
					
					maxDepth = max(maxDepth, atmosphereData.outerRadius - rayAltitude);
					...
					float c = dot(rayPosObjectSpace, rayPosObjectSpace) - (atmosphereData.outerRadius * atmosphereData.outerRadius);
					float d = (b * b) - 4.0 * a * c;
					...
					if (lightRayStepSize < atmosphereData.minStepSize) {
					...
					for (float lightRayDist = 0; lightRayDist < lightRayDepth*0.999; lightRayDist += lightRayStepSize) {
						...
						float lightRayAltitudeAboveInnerRadius = lightRayAltitude - atmosphereData.innerRadius;
						...
						vec2 lightRayDensity = exp(-max(0.001, lightRayAltitudeAboveInnerRadius) / vec2(atmosphereData.rayleighHeight, atmosphereData.mieHeight)) * lightRayStepSize;
					...
					fog += vec4(vec3(
						...
						, 0, 1) * lightIntensity
					), (atmosphereData.outerRadius - rayAltitude) / atmosphereData.atmosphereThickness * stepSize / atmosphereData.visibilityDistance);
				}
	...
	if (hasHitSomethingWithinAtmosphere) {
		ray.fog = max(ray.fog, vec4(fog.rgb, mix(0, clamp(fog.a, 0, 1), clamp(pow(smoothstep(atmosphereData.minStepSize, atmosphereData.minStepSize*100, hitDistance), 0.5),0,1))));
	} else {
