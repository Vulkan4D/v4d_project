...
void main() {
	...
	vec3 albedo = v2f.color.rgb;
	vec3 normal = v2f.normal_dist.xyz;
	float emit = 0;
	float roughness = 0.6;
	float metallic = 0.1;
	
	out_img_gBuffer_0 = vec2(metallic, roughness);
	out_img_gBuffer_1 = vec4(normal, PackUVasFloat(v2f.uv));
	out_img_gBuffer_2 = vec4(v2f.pos.xyz, v2f.normal_dist.w);
	out_img_gBuffer_3 = vec4(albedo, emit);
}
