			...
			*/
			...
			if (aabb) {
				...
				auto buffersWriteLock = entity->GetBuffersWriteLock();
					auto aabbVertices = entity->Add_proceduralVertexAABB()->AllocateBuffersCount(device, nbAabbPerChunk);
					auto vertexNormals = entity->Add_meshVertexNormal()->AllocateBuffersCount(device, nbAabbPerChunk);
					auto vertexColors = entity->Add_meshVertexColorF32()->AllocateBuffersCount(device, nbAabbPerChunk);
