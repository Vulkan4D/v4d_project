...
struct CylinderCollider : Collider {
	...
	virtual bool TerrainCollision(const std::function<double(const glm::dvec3&)>& terrainHeightMap, Entity* entity, TerrainCollisionInfo& collision) override {
		...
		points[0] = position;
		points[1] = position + rotation * glm::dvec3(NORMEPSILON,0, length/2);
		points[2] = position + rotation * glm::dvec3(0, -NORMEPSILON, -length/2);

		const glm::dvec3 downOffset = normalizedPos[0]*double(radius) * (1.0-glm::abs(glm::dot(normalizedPos[0], glm::normalize(points[1] - points[2]))));
		points[0] -= downOffset;
		points[1] -= downOffset;
		points[2] -= downOffset;
